...
if (empty($m3u_url)) {
    ...
    echo '<p style="text-align:center; font-size:12px; color:#aaa;">Optimized for Legacy Consoles</p>';
...
if (!$content) {
    ...
    echo '<p>Failed to load playlist.</p>';
...
foreach ($lines as $line) {
    ...
    if (strpos($line, '#EXTINF:') === 0) {
        ...
        if (preg_match('/group-title="([^"]+)"/', $line, $m)) $current_channel['group'] = $m[1];
        $comma_pos = strrpos($line, ',');
        if ($comma_pos !== false) $current_channel['name'] = trim(substr($line, $comma_pos + 1));
...
echo '<div class="header">Channel Guide</div>';

for ($i = $start_idx; $i < $end_idx; $i++) {
    $c = $channels[$i];
    $play_url = 'api/stream_proxy.php?url=' . urlencode(base64_encode($c['url']));
...
if ($page > 1) echo '<a class="nav-links" href="retro.php?m3u=' . urlencode($m3u_url) . '&page=' . ($page - 1) . '">&laquo; Prev</a> ';
echo ' [ Page ' . $page . ' of ' . $total_pages . ' ] ';
if ($page < $total_pages) echo '<a class="nav-links" href="retro.php?m3u=' . urlencode($m3u_url) . '&page=' . ($page + 1) . '">Next &raquo;</a>';
echo '</div>';
