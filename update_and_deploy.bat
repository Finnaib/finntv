...
echo [2/4] Rebuilding custom region m3us from new live provider list...
echo Rebuilding Egypt (egypt.m3u)...
python rebuild_egypt.py
if %errorlevel% neq 0 (
    echo Warning: Egypt rebuild had issues, check output above.
)
echo Rebuilding India, USA/UK/Canada, Sport (rebuild_all_m3u.py)...
python rebuild_all_m3u.py
if %errorlevel% neq 0 (
